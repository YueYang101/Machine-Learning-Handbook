...
### 项目文档分工与执行粒度（2026-09-08）

1. **长线路线**写最终目标、阶段依赖、研究问题和转段条件；**选型综述**写文献、数据、实现可用性、已有资产与取舍；**stage 操作说明**写这轮具体怎么执行。不要用第三份讨论稿冒充操作说明。
2. 操作说明必须有编号顺序。每步明确：前置输入／版本、要做的动作、产物文件名与归属项目、通过条件、失败时停在哪一步、当前状态。涉及 reward、扰动、成功率等，写可计算定义、单位、分母和预算，不只写「调 reward」「给干扰」「验证稳定」。
3. **既定首轮值、待标定项、后续候选必须分开**。未标定项给出测量和冻结步骤；不把每一项都写成「可以／建议／以后考虑」。变更关键参数要留协议版本、原因、受影响运行及重新冻结测试集的说明。
...
8. **抓姿来源与控制器分开比较**：人工／人类数据／生成模型／优化器可以组合，不是互斥标签。先统一物理入口与固定 controller 比候选质量，再固定初态与测试表比较闭环控制。未经配对与版本审计，不把更强执行器、更容易初态或过滤失败样本造成的增益算给方法。
