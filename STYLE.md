...
9. **以最新 PoC 边界为准。** Stage 1.1 只做单一 cube 的 Quest→retarget→L20 数采、标签与冻结划分；Stage 1.2 才用该数据比较确定性最终抓姿回归与状态条件 Flow Matching。不要把承重、稳定性、抗扰或 pick 加为这两步的前置验收。生成姿态、预测意图与稳定控制是不同验证目标，不能互相替代。
