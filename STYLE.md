...
│    ├─ 小阶段      → Stage 1.x 归属共享控制母路线，顶部集中跳转
...
Stage 1.x 以缩进子项挂在共享控制路线下，不与母路线并列为独立主线。阶段性讨论总结归属该路线，明确区分已核实事实与待决问题；实验数字仍在实验记录页维护。
...
1. **高浓度、渐进展开。** 主计划简明保留已确定的长期目标与能力主线，单独索引当前短期项目；不要为了缩短而删改长期目标或强行合并阶段。文献放独立子页，真实实验另记日志；不要把研究地图变成同时执行的任务清单。
...
9. **以最新 PoC 边界为准。** Stage 1.1 只做单一 cube 的 Quest→retarget→L20 数采、标签与冻结划分；Stage 1.2 用该数据并列测试删除 gaze／物体分支的 VQ-VAE＋自回归 Transformer，以及状态条件 Flow Matching，并以确定性最终抓姿回归作共同基线。不要把承重、稳定性、抗扰或 pick 加为这两步的前置验收。生成姿态、预测意图与稳定控制是不同验证目标，不能互相替代。
10. **长期路线与短期 stage 分开维护。** 长期目标是跨物体、跨任务、动态辅助的全程共享控制，robust grasp 只是其中一项能力。用户调整 Stage 1.1／1.2 时，只改短期方案及相关入口，不据此重排、收缩或替换长期路线；长期目标只有在用户明确要求时才变更。
11. **Stage 1.3 是预测到执行的独立验证。** 复用 1.2 模型，以单 cube 的候选选择、几何修正和目标条件闭环为起点；先基础控制对照，按失败证据升级 RL／残差，DexGen 为动作先验参考。方案记录不代表实现完成，1.3 的接触／保持指标不回填为 1.1／1.2 前置要求。
...
- **每次更新后发布线上（用户于 2026-09-08 授权）。** 完成相关检查后，将本次更新提交到 `main` 并 push 到 GitHub，检查 GitHub Pages 发布结果；无需重复询问。仅提交与本次更新相关的改动，保留无关的未提交工作。若用户明确要求仅本地修改，则该次不发布；发布失败时如实报告，不把本地修改或 push 成功当作线上已更新。
